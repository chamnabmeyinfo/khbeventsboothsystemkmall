@php
    $rolesBaseQuery = request()->query();
    $rolesBaseQuery['tab'] = 'roles';
@endphp

<div class="staff-access-tab-toolbar d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
    <h2 class="h5 mb-0 text-secondary" id="staff-access-roles-heading">Roles</h2>
...
<ul class="nav nav-tabs staff-access-sub-nav" role="tablist">
    <li class="nav-item" role="presentation">
        <a class="nav-link {{ $activeRoleSub === 'overview' ? 'active' : '' }}"
           href="{{ route('staff.access', array_merge($rolesBaseQuery, ['role_sub' => 'overview'])) }}"
           role="tab"
           aria-selected="{{ $activeRoleSub === 'overview' ? 'true' : 'false' }}">
            <i class="fas fa-chart-pie" aria-hidden="true"></i> Overview
        </a>
    </li>
    <li class="nav-item" role="presentation">
        <a class="nav-link {{ $activeRoleSub === 'cards' ? 'active' : '' }}"
           href="{{ route('staff.access', array_merge($rolesBaseQuery, ['role_sub' => 'cards'])) }}"
           role="tab"
           aria-selected="{{ $activeRoleSub === 'cards' ? 'true' : 'false' }}">
            <i class="fas fa-th-large" aria-hidden="true"></i> Card gallery
        </a>
    </li>
    <li class="nav-item" role="presentation">
        <a class="nav-link {{ $activeRoleSub === 'table' ? 'active' : '' }}"
           href="{{ route('staff.access', array_merge($rolesBaseQuery, ['role_sub' => 'table'])) }}"
           role="tab"
           aria-selected="{{ $activeRoleSub === 'table' ? 'true' : 'false' }}">
            <i class="fas fa-table" aria-hidden="true"></i> Table
        </a>
    </li>
</ul>

@if(in_array($activeRoleSub, ['cards', 'table'], true))
<div class="canvas-panel mb-3 animate-slide-up delay-2" aria-labelledby="roles-filter-heading">
    ...
    <div class="px-3 px-md-4 pb-3">
        <form method="GET" action="{{ route('staff.access') }}" id="rolesFilterForm" class="row g-3 align-items-end">
            ...
            <input type="hidden" name="role_sub" value="{{ $activeRoleSub }}">
            ...
            <div class="col-12 col-md-3 col-lg-auto d-flex flex-wrap gap-2">
                ...
                <a href="{{ route('staff.access', ['tab' => 'roles', 'role_sub' => $activeRoleSub]) }}" class="action-btn action-btn-secondary">
...
<div class="canvas-panel p-3 p-md-4 mb-0">
    <p class="text-muted mb-0">Use <strong>Card gallery</strong> for a visual grid or <strong>Table</strong> for a compact list. Filters apply on those tabs.</p>
</div>
@endif

@if($activeRoleSub === 'cards')
<div class="row g-4">
