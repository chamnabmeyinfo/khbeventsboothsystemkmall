...
<div class="row mt-4">
    <div class="col-md-12">
        ...
        <ul class="nav nav-tabs" id="settingsTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="booth-status-tab" data-bs-toggle="tab" data-bs-target="#booth-status" type="button" role="tab">
        ...
        <div class="tab-content" id="settingsTabsContent">
            ...
            <!-- Booth Status Settings Tab -->
            <div class="tab-pane fade" id="booth-status" role="tabpanel">
                <div class="card border-top-0 rounded-top-0">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h5 class="mb-1"><i class="fas fa-tags me-2"></i>Booth Booking Status Settings</h5>
                                <p class="text-muted mb-0">Customize booking status names, colors, and order. These settings will be used throughout the system.</p>
                            </div>
...
<script>
    ...
    // Load booth status settings
    function loadBoothStatusSettings() {
    ...
    function renderStatusSettings(statuses) {
        ...
        html += '<th style="width: 120px;">Background</th>';
        html += '<th style="width: 120px;">Border</th>';
        html += '<th style="width: 120px;">Text</th>';
        html += '<th style="width: 100px;">Badge</th>';
    ...
    function renderStatusRow(status, index) {
        ...
        // Background Color
        html += '<td><div class="input-group input-group-sm"><input type="color" class="form-control form-control-color status-bg-color" value="' + (status.status_color || '#28a745') + '" style="width: 50px;"><input type="text" class="form-control form-control-sm status-bg-color-text" value="' + (status.status_color || '#28a745') + '" maxlength="7"></div></td>';
        ...
        // Text Color
        html += '<td><div class="input-group input-group-sm"><input type="color" class="form-control form-control-color status-text-color" value="' + (status.text_color || '#ffffff') + '" style="width: 50px;"><input type="text" class="form-control form-control-sm status-text-color-text" value="' + (status.text_color || '#ffffff') + '" maxlength="7"></div></td>';
    ...
    // Attach event handlers for status settings
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 200) {
                    ...
                    setTimeout(function() {
                        loadBoothStatusSettings();
                        // Refresh status colors cache in canvas if it's open
                        if (typeof FloorPlanDesigner !== 'undefined' && FloorPlanDesigner.refreshStatusColors) {
                            FloorPlanDesigner.refreshStatusColors().then(function() {
                                // Reload canvas booths to apply new colors
                                if (typeof FloorPlanDesigner !== 'undefined' && FloorPlanDesigner.loadSavedPositions) {
                                    FloorPlanDesigner.loadSavedPositions();
                                }
                            });
                        }
                    }, 500);
    ...
    // Reset to defaults
    $('#btnResetStatusSettings').on('click', function() {
        if (!confirm('Reset all status settings to defaults? This will overwrite all current settings.')) {
            return;
        }
